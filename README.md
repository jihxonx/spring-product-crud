...
- 주문 목록 조회 API 구현 (페이지네이션 자유 방식)
- 응답 데이터에 **상품 이름(Product Name)** 포함
- 조회 시 **N+1 문제**가 발생하지 않도록 Fetch Join 등으로 최적화 예정
...
- 상품에 `stock` 필드 추가  
- 주문 수량(quantity)에 따라 `stock` 감소 처리  
- 재고가 0 이하일 경우 주문 불가 처리  

또한, **재고 차감이 원자적으로(Atomic)** 이루어지도록  
트랜잭션 처리를 고려하여 구현할 예정

 **구현 완료 후 확인할 내용** 
- **원자성 보장 방법** (예: `@Transactional`, Lock 등)에 대한 간단한 설명
